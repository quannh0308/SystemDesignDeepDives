...
| 2026-09-03 | Deep Dives reshaped to HelloInterview form (owner review): all titles posed as the interviewer's question (How-mechanism + Why-decision), mechanism walkthroughs added where the How was implicit (9.2 both-matchers race step by step, 9.3 no-drop delivery chain), zoomed Mermaid solution slices added to 9.1–9.4, and two missing dives written — 9.7 geohash cell boundaries (naive prefix trap → 9-cell neighbor expansion → GEOSEARCH internals; fake's pure-haversine as the correctness statement) and 9.8 the expiration path (three timers with distinct owners — engine `waitForTaskToken` 10 s, Redis PX, DDB TTL — landing on one idempotent release lane). Existing dive numbers kept stable (44 cross-references in 11 files untouched); lld traceability gains 9.7/9.8; AGENTS.md dive contract now requires question titles, mechanism walkthrough, named technology in Decision, and teaching diagrams | this commit | 8 dives, 8 receipts, fence/path audit clean |
| 2026-09-04 | Deep-dive back-propagation check (owner question: does the reshape require LLD/ledger/implementation changes?): walkthrough claims in 9.2/9.3/9.8 verified against code — all mechanisms exist and are unit-test-pinned; no runtime change needed. One genuine gap found and closed in-plan: DD 9.7's boundary claim had no scheduled verification anywhere (the in-memory fake is cell-less by design, so only real Redis can prove it) — smoke suite (tasks 7.2) gains a 5th check: a boundary pair ~200 m apart straddling a geohash cell edge must both be found by one radius search; lld §8 count updated, §4 gains the boundary-semantics cross-reference | this commit | grep audit: zero prior boundary coverage in plan/tests |
| 2026-09-04 | Final design added (owner idea, HelloInterview closing move): hld §10 — the whiteboard as it stands after all eight dives, one Mermaid diagram with named technology and dive-number annotations, plus the "what changed since §4" delta list (added: DLQ; model: rides.attempt, offer TTL = window, named guards on every transition; hardened edges: dedupe, 9-cell search, token-after-guard, untrusted cadence; deliberately unchanged: SQS/SFN/DDB/Redis). §5 rides row corrected to carry `attempt` (the dives made it part of the model). AGENTS.md hld skeleton now requires §10 | this commit | docs-only |
| 2026-09-04 | §10 walkthrough added (owner idea): "One ride, start to finish" — 12 numbered plain-words steps tracing rider C's request through D1's silent timeout, D2's accept (owner guard before token), pickup, finish, and the crash-anywhere guarantee, each step naming its carrying mechanism with dive references. AGENTS.md §10 contract now requires the walkthrough (happy path + at least one failure branch) | this commit | docs-only |
| 2026-09-04 | Walkthrough enriched to 14 steps (owner review): step 6 gains the hidden detail — D1's own availability (never wrongly busy longer than the offer window; lock TTL is the worst case); new step 8 — matcher crashes mid-offer to D2, the dangling frame described (held lock, dead token, OFFERED attempt 2) and the built-in cleanup narrated (engine catch → same release path; workflow-death flavor → store-owned TTLs + markFailed covering OFFERED; stale tap → clean 409, no ghost assignment); D3 becomes the accepting driver; closing invariant now points at the two live failure branches. One framing correction to the owner's premise recorded: the design has no unrecoverable dangling state — no janitor exists because none is needed | this commit | docs-only |
